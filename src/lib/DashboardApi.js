...
async function DashboardAPI(organizationName) {
    try {
        ...
        FROM vwArofloTaskCFOverallResult 
            WHERE completeddate >= @previousSevenDayDate AND completeddate <= @currentDate and status = @status and REPLACE(ClientName, ' ', '') IN ( SELECT REPLACE(ClientName, ' ', '') FROM [dbo].[ArofloParentChildClient] WHERE REPLACE(ParentClient, ' ', '') = REPLACE(@organizationName, ' ', '')UNION SELECT REPLACE(@organizationName, ' ', '')) AND custon <> 'TEST'  
        ...
        let upcomingJobNext7Daysdata = await pool.request()
            ...
            WHERE b.startdate > @currentDate AND b.startdate <= @nextSevenDayDate and status = @status and REPLACE(ClientName, ' ', '') IN ( SELECT REPLACE(ClientName, ' ', '') FROM [dbo].[ArofloParentChildClient] WHERE REPLACE(ParentClient, ' ', '') = REPLACE(@organizationName, ' ', '')UNION SELECT REPLACE(@organizationName, ' ', '')) AND a.custon <> 'TEST' 
