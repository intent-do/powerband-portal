...
export default async function OrganizationList() {
  try {
    ...
    //   `SELECT DISTINCT ISNULL(NULLIF(ParentClient,''), ClientName) as clientname
    //     FROM [dbo].[ArofloParentChildClient]
    //     WHERE ClientType = 'RE Agents - Resi'`
    // );
    const result = await pool.request().query(
      `SELECT DISTINCT ISNULL(NULLIF(ParentClient,''), ClientName) as clientname 
        FROM [dbo].[ArofloParentChildClient] 
        WHERE ClientType = 'RE Agents - Resi' 
        union
        SELECT DISTINCT ClientName as clientname 
            FROM [dbo].[ArofloParentChildClient] 
            WHERE ClientType = 'Businesses - Builders'`
    );
